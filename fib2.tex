...
are total charge measurements within regions bounded by %\dude{directed} 
...
%This braid relates two different ways of observing the same charges.
...
%This braid relates two different ways of observing the same charges.
%We are free to draw these loops around charges,
...
%The state \dude{space?} of a collection of anyons
%can be described by the observables of the system.
The defining %\dude{key?} 
difference between abelian and nonabelian
...
%The defining characteristic of a \emph{nonabelian}
%anyon theory is that ...
...
dimension of this space grows asymptotically as $\varphi^n$, where $\varphi=\frac{1+\sqrt{5}}{2}$ 
is the golden ratio.
...
which $F$-moves occur. A clockwise braiding process is represented by an $R$-move:
\begin{align}
\label{e:rmove}
...
%holding two anyons (punctures) with charges $a$ and $b$. \cggb{This statement isn't quite true. I suggest just deleting.}
...
%and conversely, the fusion operation acts by replacing the 
%interior of a loop by a single charge.

%Abstracted from the quantum context,
%these \emph{curve diagrams} are used in
...
%Pair-creation events should be considered isometries 
%that enlarge the fusion space, while fusion events correspond to projective measurements 
...
%Braiding of Fibonacci anyons and measurements of fusion outcomes is known to be 
%sufficient to implement universal quantum computation~\cite{Freedman2002, Nayak2008}.
% -- we already said this in the intro and we're pressed for space, so I cut this sentence 
...
%This global space is the one in which we will encode, and demonstrate protection of, quantum information. 
% -- we say this again in the paragraph after next.
...
%We tile the manifold with a finite set of $L\times L$ Wilson loops arranged in
...
%\cggb{Subtlety in that there could be anyons around in the ground space.}
...
Despite this, we use heuristics %for finite lattice size, we use heuristics
