...
\begin{figure}[t!]
\begin{center}
	\includegraphics[width=0.9\columnwidth]{pic-syndrome.pdf}\\[10pt]
	(e) \ \ \ 
...
(b) To measure the total charge contained within each tile, we must first join the fusion trees associated with all such pairs. 
...
After applying several $R$ and $F$ moves (see Eqs.~(\ref{e:rmove}) and (\ref{e:fmove})), the total charges within each tile can be read off the diagram as $q$ and $q'$.}
\label{f:syndrome}
\end{center}
\vspace{-10pt}
\end{figure}
