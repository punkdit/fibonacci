...
Each boundary component will inherit the manifold orientation,
contain a distinguished point,
and be labelled with
%an element from a finite set $\mathbb{A}=\{1, a, b, c ...\}.$
an element from a finite set $\A.$
...
labels, orientation, and distinguished points.
The modular functor associates such an $f$ to a
...
Glueing must respect the orientation of the two boundary
components as well as identifying their distinguished points.
...
is constructed from two applications of the
gluing isomorphism as follows.
...
Such a path that encloses two boundary components 
we call a \emph{2-observable}.
A function $c:[0,1]\to M$ such that $c(0)\in \partial M$ and $c(1)\in \partial M$
are two separate components of the boundary we call a \emph{2-curve.}
