...
bonds to decompose into separate clusters~\footnote{Note that we use the word cluster to refer to both groupings of nearby anyons as formed by the decoding algorithm, and also to connected components of a graph under percolation, since both of these usages are standard in the literature. This distinction should be kept in mind, though context should remove any ambiguity regarding the meaning.} of average size $O(\log(n))$ and variance 
$O(1)$~\cite{Bazant2000}.
...
%The ends of curves correspond to leaves of the fusion tree, or anyons. 
...
Although we can guarantee that error-correction will succeed whenever noise does not percolate, it is possible that percolated events may still not result in an error. The connection between the percolation threshold and the error-correction threshold is not well understood in general~\cite{Hastings2014}, though it is clear that our threshold estimate will be a lower bound for the true threshold that may have been found if all events (including those that have percolated) were simulated. However, by simulating such percolated events on small lattices, our evidence suggests that for the Fibonacci anyon code that we study, the vast majority of such events will result in a logical error. As such we feel confident that neglecting such events does not have a significant effect on the threshold value obtained.
...
code. As such, our methods could be used to demonstrate successful error-correction in an arbitrary anyonic topological quantum computation.

Applying our methods to more realistic models such as concrete microscopic spin models or models with non-topological features is an open question.
...
\stf{Is there a possibility of using MPS like Rob Pfeifer to improve the simulations or 
include interactions?}~\cite{Pfeifer2010, Pfeifer2012, Singh2014}
