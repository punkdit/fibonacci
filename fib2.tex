...
the anyons themselves that braid, but the manifold in which the anyons embed 
is considered deformed (twisted) by these braid moves.
%The generators corresponding to the braid
%group generators are called half-twists.
% -we thought this was a getting ahead of ourselves since we haven't introduced this stuff yet.
...
We imagine an idealized Hamiltonian for this system of the form
\begin{align}
	H=-\sum_{i\in \Lambda}\proj{\vac}_i\;,\label{e:hamiltonian}
\end{align}
with $\proj{\vac}_i$ the projector to charge $\vac$ at tile $i$, i.e.~the ground 
space of the model has vacuum total charge in each tile.
\dude{ouch this no longer works..}
Given the toroidal boundary 
conditions, this space is two-fold degenerate as noted above.
...
maintain the state as a separable product. \dude{???}
