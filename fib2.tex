...
Non-intersecting loops commute (why?), and so
a basis for the state can be built from a maximal
%set of non-intersecting loops.
set of disjoint and nested loops.
\emph{picture?}
%Observables measure the charge within a
%region bounded by a directed closed loop.
...
These pictures (sans quantum) are called 
\emph{curve diagrams} and are used in
...
%These manipulations encode the data of the unitary modular tensor 
%category, and can be used to represent the state of 
%a system of anyons as a linear combination of fusion trees.
...
\begin{figure}[t!]
\begin{center}
	\includegraphics[width=0.8\columnwidth]{pic-syndrome.pdf} %\\[10pt]
%	(e)\ \ 
...
%These events are 
%each represented by curves, where anyons are denoted by specific 
...
%we must first join the fusion 
%trees associated with all such pairs. 
%This corresponds to joining 
%the edges associated with each pair into a continuous path, 
%representing a basis for the fusion space chosen arbitrarily. 
...
%(e) The specific braids required to transform (c) 
%to (d), where the labels of the four leaves of 
%these fusion trees correspond to the four anyons taken from 
%the top of the path in (c) to the bottom. 
%Dashed lines denote vacuum charges. 
%After applying several $R$ and $F$ moves (see Eqs.~(\ref{e:rmove}) and 
%(\ref{e:fmove})), the total charges within each tile can be read off the diagram as $q$ and $q'$.
}
\label{f:syndrome}
\end{center}
\vspace{-10pt}
