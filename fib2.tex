...
models~\cite{Brell2013, Wootton2013, Hutter2014, Wootton2015}. Nonabelian anyon models are especially interesting 
...
based on a hierarchical clustering algorithm~\cite{Hastie2009, Wootton2015},
...
\begin{center}
		...
		\begin{tikzpicture}[baseline=0.2cm,scale=0.3,xscale=0.9]		
			...
			\end{scope}
		\end{tikzpicture}
\caption{In order to transform from the configuration shown in \Fref{f:syndrome}(c) to \ref{f:syndrome}(d), several operations must be performed. The labels of the four leaves of these fusion trees correspond to the four anyons taken from the top of the path in \Fref{f:syndrome}(c) to the bottom. Vacuum charges are denoted by dashed lines. In order to rearrange the anyons so that particles 1 and 4 are neighbors, several braid moves must be performed as shown. The effect of these on the fusion tree can then be evaluated by the application of several $R$ and $F$ moves as defined in equations (\ref{e:rmove}) and (\ref{e:fmove}). After the manipulations shown, the total charges within each tile can be read off the diagram as $q$ and $q'$.}
\label{f:syndrome2}
\end{center}
\end{figure}


Following the result of these charge measurements, the decoder determines a recovery operation that involves fusion of subsets of anyons. These fusion results can be calculated in an analogous way, and the output charge placed at an appropriate point in the lattice. This procedure is iterated until either the decoder terminates successfully, or the simulation itself declares failure.
