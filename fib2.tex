...
description of the curve(s) intersected with that cell.
This data structure 
will be insensitive to any continuous deformation of the interior of the cell,
...
we record our encounters with each piece of curve diagram,
...
The words for the above two cells will then be 
...
the curve with the cell boundary,
...
eg. $\bigl<_{\mbox{\tt in}}\bigl<_{\mbox{\tt out}}\bigr>_{\mbox{\tt in}}\bigr>_{\mbox{\tt out}}\bigl<_{\mbox{\tt out}}\bigr>_{\mbox{\tt in}}$.
Similarly, the decorated symbols $*_{\mbox{\tt in}}$ and 
$*_{\mbox{\tt out}}$ 
will denote respectively either
the head, $c(1)$ or the tail $c(0)$ of a curve.

{\bf (II)} We will record,
for each intersection with the cell edge, which of the four
sides of the cell the
...
This will require every curve diagram to intersect 
the edges of cells transversally,
and in particular a curve will not touch a cell corner.
...
Various other consistency relations are enforced on such a combinatorial curve diagram,
