...
For a more pedestrian treatment, see \cite{Freedman2002simulation} or \cite{Beverland2004.}

For our purposes a \emph{surface} will be an
oriented $2$-dimensional manifold with boundary.
Each boundary component is labelled with
an element from a finite set $\mathbb{A}=\{1, a, b, c ...\}.$
This is the set of ``anyon labels'', and comes
equiped with an involution $\hat{}$ such that $\hat{1}=1.$
%Each boundary component is oriented (either explicitly or
%otherwise by using the orientation of the manifold)
Each boundary component inherits the manifold orientation 
and contains a distinguished point.

We will mostly be concerned with \emph{planar} such
surfaces, ie. disc(s) with zero or more holes.
Note that we will not require surfaces themselves to be connected.
...
a topological homeomorphism that preserves boundary
labels and orientation, and
the modular functor assocates such an $f$ to a
unitary transformation $\F(f) : \F(M)\to\F(N).$
Functoriality requires that $\F$
respect composition of homeomorphisms.
...
{\bf Axiom III.}
%Gluing.
Denote a surface $M$ with (at least) two boundary
components labelled $a, b$ as $M_{a,b}$. 
Let $N$ be the surface obtained from $M$ by gluing
these two boundary components together.
If we constrain $b=\hat{a}$ then the fusion space
of $M_{a,\hat{a}}$ embedds unitarily in the fusion
space of $N.$ Moreover, there is a unitary map
$$
    \bigoplus_{a\in\mathbb{A}} \F(M_{a,\hat{a}}) \xrightarrow{\cong} \F(N).
$$
...
Here we show this process applied to the disjoint union
%of two surfaces $M_a$ and $N_{\hat{a}}$. 
of two surfaces $M$ and $N$. 
We display the surface $N$ with the $\hat{a}$
boundary on the outside, to show more clearly how $N$ fits into $M$.
In the glued manifold we
indicate the placement of $M$ and $N$ and the seam
along which glueing occured.
\begin{center}
\includegraphics[]{pic-glue.pdf}
\end{center}

\emph{XXX show marked point on boundaries ?}
