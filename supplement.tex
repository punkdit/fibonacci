...
\section{Modelling and simulating error and recovery}\label{secAlgo}
...
As noted above, the key to our model is to represent anyons and topological operations with curve diagrams. A curve diagram is a directed path that connects several anyons, which are denoted by preferred points along these paths. A curve diagram specifies a linear ordering of the anyons that allows us to define a basis for the corresponding fusion space, as well as to keep track of topological operations on this ordering (details are provided in section \ref{secAbstract}).

\cggb{Maybe here a picture of a generic curve with some anyons on it and a linear ordering of the anyons in a fusion diagram type picture.}

The main noise processes we consider in our simulations are pair-creation events, though hopping, exchange, etc.~processes can also be implemented straightforwardly.
Pair-creation processes act to populate the manifold with
...
In calculating the tile observable measurement outcomes, it will be convenient to consider anyons being transported around the lattice
by moving them along tile edges. This is equivalent to a "refactoring" of the fusion space, as described in section \ref{secAbstract}.
...
%
%The decoder measures successively larger and larger
%regions of the lattice
%until there are no more charges 
%or a topologically non-trivial operation has occurred
...
\end{center}
\cggb{Maybe replace "measure" in this picture with "recovery"?}

So far we have discussed the simulation of the (quantum) system
and now we turn to the decoder algorithm. This is similar to other clustering-type decoders used in the literature~\cite{??}, in that it attempts to form and fuse together clusters of anyons on larger and larger length scales, declaring failure if anyons remain after fusion at the largest length scale. Our simulation also declares failure if at any point the curve diagrams describing the state of the system form a topologically non-trivial loop (span the lattice), this being a necessary condition for error-correction failure.
...
fusing with the charge at the root. This movement of anyons is a physical recovery operation that is simulated as described above.
\begin{center}
...
In this section we will describe abstractly the theory of two-dimensional topologically ordered systems
...
explicitly used in Refs.~\cite{Freedman2002, Freedman2002simulation, Beverland2014, Kitaev2006topo} (in some of these references modular functors are simply called TQFTs).
...
In this section we will sketch how a 
...
\end{center}
For identical particles this is a symmetry of the system. \cggb{What does symmetry mean here? Since a swap of non-bosons will pick up a phase.}
...
\cggb{I was a bit confused by when we care about unlabelled configuration space and when we care about the labelled configuration space. What if we have some but not all particles identical? Do the two formalisms unify by considering some subgroups of the permutation group that preserve particle label or something?}
$$
    \UConf_n = \Conf_n / S_n.
$$
...
%We denote the two-dimensional space 
%The two dimensional space within which the particles reside we denote 
We pick an $n$-point finite subset $Q_n \subset M$
and consider diffeomorphisms \cggb{why diffeomorphisms and not homeomorphisms?} $f : M \to M$
...
where we show a \emph{framing} which is a non-degenerate \cggb{What's a non-degenerate vector field? What goes wrong if its degenerate?}
...
manifold orientation, hole labels, and base points. \cggb{"preserves base points" might be misleading or too specific.}
...
class of $f$ under isotopy is called the \emph{isotopy class} of $f.$ \cggb{I didn't quite see why this more formal definition is the same as equivalent up to continuous deformation. Why can I restrict to maps where the base points are fixed along the whole path?}
...
and we have only pair-of-pants. \cggb{Is this always possible? What if I have something like a cylinder? Then I just get a bunch of annuli however I cut.}
...
which is essentially the same as {skein theory.} \cggb{Is all this curve diagrams and refactoring theorem and and so on original work, or is it collecting known facts? If its original, this should be an independent paper because otherwise nobody from the math phys community will ever find it. Actually maybe in either case it should be an independent arxiv post at least so that more people find and learn from it.}
...
removed. \cggb{Why is this easier to describe on $\mathbb{C}$ rather than $\mathbb{R}^2$? If its all the same, I think it could be safer to rephrase on $\R^2$.}
...
that fixes the real axis while moving the surface to a standard surface. \cggb{Does "fix the real" axis here mean that it fixes every point on the real axis, or just maps the real axis to itself?}
%while moving the holes so as to produce
%a standard surface.

\subsub{Curve diagram.}
We next study maps from standard surfaces to arbitrary surfaces. \cggb{I was a bit confused what this means. Maps means between surfaces with the same holes and base points, right? So then don't we only care about maps from standard surfaces to standard surfaces? Is it maybe more like maps from equatorial curve diagrams on standard surfaces to arbitrary curve diagrams on standard surfaces that we want?}
...
the restriction of $f$ to the equator of $\StdM.$ \cggb{citation? or obvious?}
...
the refactoring theorem below. \cggb{citation? or original work?} %\simon{XXX} 
...
diagram. \cggb{Is respecting the equator the same as respecting whatever canonical curve diagram we choose?}
...
not just two but any even number of curve intersections at each hole. \cggb{What's a planar algebra and is there an easier place for me to learn about it than from the Jones paper?}
...
calculations. \cggb{Do we need to worry about this if we are pasting other surfaces (whose total charge might be ambiguous) into a hole?}
...
the $R$-moves act transitively \cggb{what do we mean here by transitively} on curve diagrams.
...
The combinatorial ``paperclip algorithm'' presented in section \ref{secAlgo} can be considered to be a constructive reformulation of the refactoring theorem, where a curve diagram is represented as a function
